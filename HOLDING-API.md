...
| `X-Holding-Tenant`  | Slug tenant (`domain` atau `domain_alt`) | `usaha.domain` atau `usaha.domain_alt`     |

### Cara mendapatkan kredensial

1. Login ke **Master SIDBM** (`/master`).
2. Buka menu **License** di sidebar.
3. **Tambah License** untuk usaha target:
   - Pilih usaha dari dropdown.
...
5. Kirim `X-Holding-Tenant` = `domain` atau `domain_alt` dari tabel `usaha` untuk tenant tsb.
...
```json
{
  ...
  "ringkasan": {
    "point_a": "Per 30 Juni 2025, kondisi keuangan Usaha ABC...",
    "total_aset": 150000000.0,
    "total_liabilitas_ekuitas": 150000000.0,
    "selisih": 0.0
  },
...
$client = new HoldingLaporanClient(
    ...
    'app.sidbm.net'                                 // domain atau domain_alt
);

$neraca = $client->neraca(2025, 6, 30);
echo "Usaha: {$neraca['kecamatan']}\n";
...
3. `usaha.domain` atau `domain_alt` cocok dengan header `X-Holding-Tenant`
4. Tidak ada spasi/newline tak terlihat di `api_secret` (paste dari notepad kadang bawa newline)

**Debug query:**

```sql
SELECT l.id, l.api_secret, l.is_active, l.expired_at, k.domain, k.domain_alt
FROM licenses l JOIN usaha k ON k.id = l.usaha_id
...
2. `X-Holding-Tenant` cocok dengan `domain` / `domain_alt` di tabel `usaha`
...
```sql
-- Tabel usaha (atau "tenant" / "lembaga" — nama bisa disesuaikan)
CREATE TABLE usaha (
  id              BIGINT PRIMARY KEY AUTO_INCREMENT,
  nama_kec        VARCHAR(100) NOT NULL,
  kd_kec          VARCHAR(20),                  -- kode BPS / wilayah
  domain         VARCHAR(100) UNIQUE,          -- URL primer untuk X-Holding-Tenant
  domain_alt  VARCHAR(100) UNIQUE,          -- URL alternatif (opsional, juga valid)
  -- field lain sesuai bisnis...
);

-- Tabel license (token per usaha, 1 tenant = 1 license)
CREATE TABLE licenses (
  id            BIGINT PRIMARY KEY AUTO_INCREMENT,
  usaha_id  BIGINT NOT NULL UNIQUE,         -- 1 license per usaha
  api_secret    VARCHAR(255) NOT NULL,
  is_active     TINYINT(1) NOT NULL DEFAULT 1,
  expired_at    DATETIME NULL,                  -- NULL = tidak ada batas waktu
  created_at    TIMESTAMP NULL,
  updated_at    TIMESTAMP NULL,
  FOREIGN KEY (usaha_id) REFERENCES usaha(id)
);
...
- `domain` / `domain_alt` di-allow null tapi untuk holding harus unique.
...
def holding_license_middleware(request, next_handler):
    ...
    # 2. Lookup license + usaha (1 query)
    license = db.query("""
        SELECT l.*, k.id as kec_id, k.domain, k.domain_alt
        FROM licenses l
        JOIN usaha k ON l.usaha_id = k.id
        WHERE l.api_secret = :token
          AND l.is_active = 1
          AND (l.expired_at IS NULL OR l.expired_at > NOW())
          AND (k.domain = :slug OR k.domain_alt = :slug)
        LIMIT 1
...
**Implementasi Laravel (suffix via middleware):**
...
public function handle(Request $request, Closure $next)
{
    ...
    // Kunci: pakai usaha->id sebagai suffix, BUKAN nama
    config(['tenant.suffix' => '_' . $license->usaha->id]);
...
> ⚠️ **Penting:** suffix tabel = `id` usaha, BUKAN `domain` / `domain_alt`. `id` adalah integer cocok untuk suffix; slug bisa ada karakter non-aman.
...
- [ ] **Unique index** pada `licenses.api_secret` dan `usaha.domain` / `domain_alt`
...
async function holdingLicense(req, res, next) {
    ...
    const [rows] = await pool.query(`
        ...
        FROM licenses l JOIN usaha k ON l.usaha_id = k.id
        WHERE l.api_secret = ? AND l.is_active = 1
          AND (l.expired_at IS NULL OR l.expired_at > NOW())
          AND (k.domain = ? OR k.domain_alt = ?)
        LIMIT 1
    `, [token, slug, slug]);
    ...
    req.tenant = {usaha_id: rows[0].kec_id, license_id: rows[0].id};
...
class HoldingLicenseMiddleware:
    ...
    def __call__(self, request):
        ...
        license = License.objects.select_related('usaha').filter(
            api_secret=token,
            is_active=True,
        ).filter(
            Q(expired_at__isnull=True) | Q(expired_at__gt=timezone.now())
        ).filter(
            Q(usaha__domain=slug) | Q(usaha__domain_alt=slug)
        ).first()
        ...
        request.tenant = {'usaha_id': license.usaha.id}
        connection.schema_name = f'tenant_{license.usaha.id}'  # django-tenants
...
❌ **Suffix tabel dari `domain` / slug.** Pakai integer `id`. Slug bisa berubah / punya karakter aneh.
